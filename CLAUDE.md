...
- **A deployed beta is live** at commit `b9d1c5b`: [production](https://epl-como-fantasy-26-27-cgtd.vercel.app). Core auction flow, corrections, trades, sealed reveals and viewer surfaces are implemented. The complete v1 recap, read-only trade log, #55 phone fix, additive schema migrations, stage tags and expanded standard player stats are live. Production `/api/recap` returns 200 and the browser renders awards, final squads, FPL Draft checklists and the ledger link; #56 is closed. The integrated runner now has 16 suites, including the read-only trades suite. Remaining Aug 2 gates are production reset/freeze, the post-reset audit and the formal two-device, load and physical/fallback rehearsal.
