...
> Status: source and recorded evidence reconciled 15 Jul 2026 at commit `b9d1c5b`. Issues #48 and #50 remain the recorded isolated 15-suite baseline; the landed read-only trades work adds a sixteenth suite and reports the full battery green. Runtime observations below are dated evidence, not an assertion of continuing service health.
...
| 9 | Config change honoured with no code change | ✅ loader tests + deployed roster override · 🔜 full variant app boot | `test-config` now has 22 checks, including env-only override, file precedence and malformed env failure. The 11 Jul production observation showed the real eight-manager override from `LEAGUE_CONFIG_LOCAL`. Still boot the full app against a variant budget, manager count and squad shape before production acceptance |
| 10 | Port walk completes with no code change | partial: deployment recorded · 🔜 operational + formal closure | Vercel recorded a successful deployment of `b9d1c5b` against Neon with the real override. Record in `docs/PORTING.md` and issue #23 that no further source change was required after the known roster loader, then complete the mutation and fallback evidence |
| 11 | Browser smoke test on the deployed URL, human-confirmed | partial: read-only observation · 🔜 formal two-device mutation | A human observed the board, recap, trades log and player detail routes rendering on 11 Jul. Still use two physical devices to record a sale with the token, watch the board update within ~2s and the reveal fire, then undo it. Capture the result in issue #23 |
...
Recorded evidence at `b9d1c5b`: Vercel completed the deployment; the integrated runner contains 16 suites; #55 and #56 are closed; and the recap, trades log and expanded standard player detail were observed in production on 11 Jul. Neon, the real roster override and the earlier read-only payload/token audit were also observed. Formal production reset/freeze, post-reset audit, two-device mutation, 10-tab sustained-load run, physical TV check, hotspot drill, laptop fallback and cold-standby restore are pending.
...
7. **Production recap smoke:** recorded observation from 11 Jul at `b9d1c5b`: `/api/recap` returned 200 and `/recap` rendered awards, war chests, final squads, FPL Draft checklists and the ledger link. Repeat after production reset/freeze; the dated observation is not ongoing health evidence.
